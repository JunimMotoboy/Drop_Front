...
function loadUserInfo() {
    ...
    if (user) {
        ...
        if (user.criado_em) {
            ...
            document.getElementById('stat-membro-desde').textContent = date.toLocaleDateString('pt-BR');
        }
    }
}
...
async function loadEntregas() {
    try {
        const response = await fetchWithAuth(`${API_URL}/encomendas/minhas-entregas`, {
            method: 'GET'
        });
...
function renderEntregas() {
    ...
    const pendentes = entregas.filter(e => e.status !== 'entregue' && e.status !== 'cancelado');
    ...
    container.innerHTML = pendentes.map(entrega => {
        const status = formatStatus(entrega.status);
        const tipoEntrega = entrega.tipo_entrega === 'agendada' ? 'Agendada' : 'Móvel';
        
        return `
            <div class="entrega-item status-${entrega.status}" onclick="verDetalhes(${entrega.id_encomenda})">
                <div class="entrega-icon">
                    <i class="fas fa-box"></i>
                </div>
                <div class="entrega-content">
                    <div class="entrega-header">
                        <div class="entrega-codigo">#${entrega.codigo_rastreio || entrega.id_encomenda}</div>
                        <span class="entrega-status ${status.class}">${status.text}</span>
                    </div>
                    <div class="entrega-info">
                        ...
                        <div class="info-row">
                            ...
                            <span>R$ ${parseFloat(entrega.valor).toFixed(2)}</span>
                        </div>
                        <div class="info-row">
                            <i class="fas fa-truck"></i>
                            <span>${tipoEntrega}</span>
                        </div>
                        ${entrega.tipo_entrega === 'agendada' && entrega.data_agendada ? `
                            <div class="info-row">
                                <i class="fas fa-calendar"></i>
                                <span>${formatDate(entrega.data_agendada)}</span>
                            </div>
                        ` : ''}
                    </div>
                </div>
                <div class="entrega-actions">
                    <button class="btn btn-primary btn-sm" onclick="event.stopPropagation(); verDetalhes(${entrega.id_encomenda})">
                        Ver Detalhes
                    </button>
                </div>
            </div>
...
function filterEntregas() {
    ...
    const filtered = entregas.filter(entrega => {
        ...
        return matchStatus && matchTipo && entrega.status !== 'entregue' && entrega.status !== 'cancelado';
    });
...
async function verDetalhes(idEncomenda) {
    ...
    document.getElementById('det-codigo').textContent = currentEntrega.codigo_rastreio || `#${currentEntrega.id_encomenda}`;
    document.getElementById('det-cliente').textContent = currentEntrega.nome_cliente;
    document.getElementById('det-loja').textContent = currentEntrega.loja_origem;
    document.getElementById('det-valor').textContent = `R$ ${parseFloat(currentEntrega.valor).toFixed(2)}`;
    
    const status = formatStatus(currentEntrega.status);
    document.getElementById('det-status').innerHTML = `<span class="entrega-status ${status.class}">${status.text}</span>`;
    
    document.getElementById('det-tipo').textContent = currentEntrega.tipo_entrega === 'agendada' ? 'Agendada' : 'Móvel';

    // Endereço e agendamento
    if (currentEntrega.tipo_entrega === 'agendada') {
        document.getElementById('det-endereco').textContent = currentEntrega.endereco_entrega || 'Não informado';
        document.getElementById('det-endereco-container').style.display = 'flex';
        
        if (currentEntrega.data_agendada) {
            document.getElementById('det-agendamento').textContent = formatDate(currentEntrega.data_agendada);
            document.getElementById('det-agendamento-container').style.display = 'flex';
        } else {
...
function toggleLocationSharing() {
    if (isLocationActive) {
        ...
        document.getElementById('location-text').textContent = 'Localização desativada';
        document.getElementById('btn-toggle-location').innerHTML = '<i class="fas fa-map-marker-alt"></i> Ativar Localização';
        ...
        if ('geolocation' in navigator) {
            ...
            document.getElementById('btn-toggle-location').innerHTML = '<i class="fas fa-map-marker-alt"></i> Desativar Localização';
